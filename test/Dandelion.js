...
const assertRevert = require("@aragon/templates-shared/helpers/assertRevert")(
  web3
);
...
contract("Dandelion", ([_, owner, holder1, holder2, notHolder, someone]) => {
  ...
  const newBaseInstance = (...params) => {
    ...
    const txParams =
      !Array.isArray(lastParam) && typeof lastParam === "object"
        ? params.pop()
        : {};
    const newInstanceFn = DandelionTemplate.abi.find(
      ({ name, inputs }) =>
        name === "newBaseInstance" && inputs.length === params.length
    );
    return template.sendTransaction(
      encodeCall(newInstanceFn, params, txParams)
    );
  };
  ...
  const newTokenAndBaseInstance = (...params) => {
    ...
    const newInstanceFn = DandelionTemplate.abi.find(
      ({ name, inputs }) =>
        name === "newTokenAndBaseInstance" && inputs.length === params.length
    );
    return template.sendTransaction(
      encodeCall(newInstanceFn, params, txParams)
    );
  ...
  const installDandelionApps = (...params) => {
    ...
    const newInstanceFn = DandelionTemplate.abi.find(
      ({ name, inputs }) =>
        name === "installDandelionApps" && inputs.length === params.length
    );
    return template.sendTransaction(
      encodeCall(newInstanceFn, params, txParams)
    );
  ...
  const loadDAO = async (
    tokenReceipt,
    instanceReceipt,
    dandelionAppsReceipt,
    apps = { vault: false, agent: false }
  ) => {
    dao = Kernel.at(getEventArgument(instanceReceipt, "DeployDao", "dao"));
    token = MiniMeToken.at(
      getEventArgument(tokenReceipt, "DeployToken", "token")
    );
    ...
    assert.equal(
      dao.address,
      getEventArgument(dandelionAppsReceipt, "SetupDao", "dao"),
      "should have emitted a SetupDao event"
    );

    // Base apps
    assert.equal(
      installedApps.finance.length,
      1,
      "should have installed 1 finance app"
    );
    finance = Finance.at(installedApps.finance[0]);

    assert.equal(
      installedApps["token-manager"].length,
      1,
      "should have installed 1 token manager app"
    );
    ...
    if (apps.agent) {
      assert.equal(
        ...
        1,
        "should have installed 1 agent app"
      );
    ...
    if (apps.vault) {
      assert.equal(
        installedApps.vault.length,
        1,
        "should have installed 1 vault app"
      );
    ...
    // Dandelion apps
    assert.equal(
      installedApps["dandelion-voting"].length,
      1,
      "should have installed 1 dandelion voting app"
    );
    dandelionVoting = DandelionVoting.at(installedApps["dandelion-voting"][0]);

    assert.equal(
      installedApps["redemptions"].length,
      1,
      "should have installed 1 redemptions app"
    );
    redemptions = Redemptions.at(installedApps["redemptions"][0]);

    assert.equal(
      installedApps["token-request"].length,
      1,
      "should have installed 1 token request app"
    );
    tokenRequest = TokenRequest.at(installedApps["token-request"][0]);

    assert.equal(
      installedApps["time-lock"].length,
      1,
      "should have installed 1 time lock app"
    );
    timeLock = TimeLock.at(installedApps["time-lock"][0]);

    assert.equal(
      installedApps["token-balance-oracle"].length,
      1,
      "should have installed 1 token balance oracle app"
    );
    tokenBalanceOracle = TokenBalanceOracle.at(
      installedApps["token-balance-oracle"][0]
    );
  };
  ...
  const itSetupsDAOCorrectly = (agentAsVault, financePeriod) => {
    it("registers a new DAO on ENS", async () => {
      ...
      const resolvedAddress = await PublicResolver.at(
        await ens.resolver(aragonIdNameHash)
      ).addr(aragonIdNameHash);
      assert.equal(
        resolvedAddress,
        dao.address,
        "aragonId ENS name does not match"
      );
    });
    ...
    it("mints requested amounts for the holders", async () => {
      assert.equal(
        ...
        STAKES.reduce((a, b) => a + b)
      );
      for (const holder of HOLDERS)
        assert.equal(
          (await token.balanceOf(holder)).toString(),
          STAKES[HOLDERS.indexOf(holder)]
        );
    });
    ...
    it("should have finance app correctly setup", async () => {
      ...
      assert.equal(
        (await finance.getPeriodDuration()).toString(),
        expectedPeriod,
        "finance period should be 30 days"
      );
    ...
    it("should have dandelion voting app correctly setup", async () => {
      assert.isTrue(
        await dandelionVoting.hasInitialized(),
        "voting not initialized"
      );
      assert.equal(
        (await dandelionVoting.supportRequiredPct()).toString(),
        SUPPORT_REQUIRED
      );
      assert.equal(
        (await dandelionVoting.minAcceptQuorumPct()).toString(),
        MIN_ACCEPTANCE_QUORUM
      );
      assert.equal(
        (await dandelionVoting.voteDurationBlocks()).toString(),
        VOTE_DURATION
      );
      assert.equal(
        (await dandelionVoting.voteBufferBlocks()).toString(),
        VOTE_BUFFER
      );
      assert.equal(
        (await dandelionVoting.executionDelayBlocks()).toString(),
        VOTE_DELAY
      );
    ...
    it("should have redemptions app correctly setup", async () => {
      ...
      assert.isTrue(
        await redemptions.hasInitialized(),
        "redemptions not initialized"
      );
      assert.deepStrictEqual(
        await redemptions.getRedeemableTokens(),
        REDEEMABLE_TOKENS
      );
      assert.equal(
        web3.toChecksumAddress(await redemptions.tokenManager()),
        tokenManager.address,
        "token manager not linked to redemptions app"
      );
      assert.equal(
        web3.toChecksumAddress(await redemptions.vault()),
        agentOrVault.address,
        "vault not linked to redemptions app"
      );
      ...
      await assertRole(acl, redemptions, dandelionVoting, "REDEEM_ROLE", {
        address: ANY_ENTITY
      }); // TODO: check oracle
    });
    ...
    it("should have token request app correctly setup", async () => {
      ...
      assert.isTrue(
        await tokenRequest.hasInitialized(),
        "token request not initialized"
      );
      assert.deepStrictEqual(
        await tokenRequest.getAcceptedDepositTokens(),
        ACCEPTED_DEPOSIT_TOKENS
      );
      assert.equal(
        web3.toChecksumAddress(await tokenRequest.tokenManager()),
        tokenManager.address,
        "token manager not linked to token request app"
      );
      assert.equal(
        web3.toChecksumAddress(await tokenRequest.vault()),
        agentOrVault.address,
        "vault not linked to token request app"
      );
    ...
    it("should have time lock app correctly setup", async () => {
      ...
      assert.equal(
        (await timeLock.spamPenaltyFactor()).toString(),
        SPAM_PENALTY_FACTOR
      );
    ...
    it("should have token oracle app correctly setup", async () => {
      assert.isTrue(
        await tokenBalanceOracle.hasInitialized(),
        "token oracle not initialized"
      );
      assert.equal(
        web3.toChecksumAddress(await tokenBalanceOracle.token()),
        web3.toChecksumAddress(await tokenManager.token())
      );
      assert.equal(
        (await tokenBalanceOracle.minBalance()).toString(),
        bigExp(1, 18)
      );
  ...
  const itSetupsAgentAppCorrectly = () => {
    it("should have agent app correctly setup", async () => {
      ...
      assert.equal(
        await dao.recoveryVaultAppId(),
        APP_IDS.agent,
        "agent app is not being used as the vault app of the DAO"
      );
      assert.equal(
        web3.toChecksumAddress(await finance.vault()),
        agent.address,
        "finance vault is not linked to the agent app"
      );
      assert.equal(
        web3.toChecksumAddress(await dao.getRecoveryVault()),
        agent.address,
        "agent app is not being used as the vault app of the DAO"
      );
  ...
  const itSetupsVaultAppCorrectly = () => {
    it("should have vault app correctly setup", async () => {
      ...
      assert.equal(
        await dao.recoveryVaultAppId(),
        APP_IDS.vault,
        "vault app is not being used as the vault app of the DAO"
      );
      assert.equal(
        web3.toChecksumAddress(await finance.vault()),
        vault.address,
        "finance vault is not the vault app"
      );
      assert.equal(
        web3.toChecksumAddress(await dao.getRecoveryVault()),
        vault.address,
  ...
  context("creating instances with separated transactions", () => {
    ...
    context("when the creation succeeds", () => {
      ...
      const itCostsUpTo = (
        expectedDaoCreationCost,
        expectedDandelionAppsInstallationCost
      ) => {
        const expectedTokenCreationCost = 1.8e6;
        const expectedTotalCost =
          expectedTokenCreationCost +
          expectedDaoCreationCost +
          expectedDandelionAppsInstallationCost;
        ...
        it(`gas costs must be up to ~${expectedTotalCost} gas`, async () => {
          ...
          assert.isAtMost(
            tokenCreationCost,
            expectedTokenCreationCost,
            `token creation call should cost up to ${tokenCreationCost} gas`
          );
          ...
          assert.isAtMost(
            daoCreationCost,
            expectedDaoCreationCost,
            `dao creation call should cost up to ${expectedDaoCreationCost} gas`
          );

          const dandelionAppsInstallationCost =
            dandelionAppsReceipt.receipt.gasUsed;
          assert.isAtMost(
            dandelionAppsInstallationCost,
            expectedDandelionAppsInstallationCost,
            `install dandelion apps call should cost up to ${expectedDandelionAppsInstallationCost} gas`
          );

          const totalCost =
            tokenCreationCost + daoCreationCost + dandelionAppsInstallationCost;
          assert.isAtMost(
            totalCost,
            expectedTotalCost,
            `total costs should be up to ${expectedTotalCost} gas`
          );
        });
      };
      ...
      const createDAO = (useAgentAsVault = false, financePeriod = 0) => {
        before("create Dandelion", async () => {
          ...
          instanceReceipt = await newBaseInstance(
            daoID,
            HOLDERS,
            STAKES,
            financePeriod,
          ...
          await loadDAO(tokenReceipt, instanceReceipt, dandelionAppsReceipt, {
            vault: !useAgentAsVault,
            agent: useAgentAsVault
          });
        });
      };
  ...
  context("creating instances with one transaction", () => {
    ...
    context("when the creation succeeds", () => {
      ...
      const itCostsUpTo = (
        ...
        const expectedTotalCost =
          expectedDaoCreationCost + expectedDandelionAppsInstallationCost;
      ...
      const createDAO = (useAgentAsVault = false, financePeriod = 0) => {
        before("create Dandelion", async () => {
          timeLockToken = await ERC20.new(owner, "Lock Token", "LKT");
          daoID = randomId();
          instanceReceipt = await newTokenAndBaseInstance(
            TOKEN_NAME,
            TOKEN_SYMBOL,
            daoID,
            HOLDERS,
          ...
          await loadDAO(
            ...
            instanceReceipt,
            dandelionAppsReceipt,
            { vault: !useAgentAsVault, agent: useAgentAsVault }
          );
        });
      };
